...
public class DummyWaitingQuartzJob implements Job {

    private final TaskService taskService;

    public DummyWaitingQuartzJob(TaskService taskService) {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        log.info("DummyWaitingQuartzJob is triggered for topic: {}", topic);

        TaskEntityDto task = taskService.createTask(topic, topic + "-quartz-" + LocalDateTime.now());

        log.info("DummyWaitingQuartzJob is finished for topic: {} and created entity: {}", topic, task);
    }
}
