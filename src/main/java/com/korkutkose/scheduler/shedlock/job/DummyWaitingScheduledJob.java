 ...
 */
@Slf4j
public class DummyWaitingScheduledJob implements Runnable {

    private final TaskService taskService;
    private final String topic;

    public DummyWaitingScheduledJob(TaskService taskService, String topic) {
    ...
    public void run() {
        ...
        log.info("DummyWaitingScheduledJob is triggered for topic: {}", topic);
        TaskEntityDto task = taskService.createTask(topic, topic + "-shedlock-" + LocalDateTime.now());
        log.info("DummyWaitingScheduledJob is finished for topic: {} and created entity: {}", topic, task);
    }
}
